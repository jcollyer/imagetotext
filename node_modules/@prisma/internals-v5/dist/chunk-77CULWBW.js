...
var chunk_77CULWBW_exports = {};
__export(chunk_77CULWBW_exports, {
...
module.exports = __toCommonJS(chunk_77CULWBW_exports);
...
async function formatSchema({ schemaPath: schemaPath2, schema }, inputFormattingOptions) {
  ...
  const { formattedSchema, lintDiagnostics } = handleFormatPanic(
    () => {
      ...
      const lintDiagnostics2 = (0, import_chunk_Y3WKXJ4S.lintSchema)({ schema: formattedSchema2 });
      return { formattedSchema: formattedSchema2, lintDiagnostics: lintDiagnostics2 };
    },
    { schemaPath: schemaPath2, schema }
  );
  const lintWarnings = (0, import_chunk_Y3WKXJ4S.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_TTXZEUOA.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
...
async function getGenerators(options) {
  ...
  const config = await (0, import_chunk_HYWHXF4Y.getConfig)({
    datamodel,
  ...
  const dmmf = await (0, import_chunk_7HBIGZVE.getDMMF)({
    datamodel,
    datamodelPath: schemaPath,
    prismaPath,
    previewFeatures
...
`
    ...
    const binaryPathsByVersion = await (0, import_chunk_XF4XAB7W.getBinaryPathsByVersion)({
      neededVersions,
      binaryTarget,
      version,
      printDownloadProgress,
    ...
    for (const generator of generators) {
      if (generator.manifest && generator.manifest.requiresEngines) {
        ...
        if (engineVersion !== version && generator.options && generator.manifest.requiresEngines.includes(queryEngineType) && generatorBinaryPaths[queryEngineType] && generatorBinaryPaths[queryEngineType]?.[binaryTarget]) {
          const customDmmf = await (0, import_chunk_7HBIGZVE.getDMMF)({
            datamodel,
            datamodelPath: schemaPath,
...
async function checkUnsupportedDataProxyMessage(command, args, implicitSchema) {
  ...
  for (const [argName, argValue] of argList) {
    ...
    if (argName.includes("schema")) {
      ...
      const config2 = await (0, import_chunk_HYWHXF4Y.getConfig)({ datamodel: datamodel2, ignoreEnvVarErrors: true });
      const url = (0, import_chunk_HYWHXF4Y.resolveUrl)((0, import_chunk_HYWHXF4Y.getEffectiveUrl)(config2.datasources[0]));
