...
var chunk_7HBIGZVE_exports = {};
__export(chunk_7HBIGZVE_exports, {
  GetDmmfError: () => GetDmmfError,
  getDMMF: () => getDMMF
});
module.exports = __toCommonJS(chunk_7HBIGZVE_exports);
...
var GetDmmfError = class extends Error {
  constructor(params) {
    const constructedErrorMessage = (0, import_chunk_KTERFAQE.N)(params).with({ _tag: "parsed" }, ({ errorCode, message, reason }) => {
      const errorCodeMessage = errorCode ? `Error code: ${errorCode}` : "";
      return `${reason}
${errorCodeMessage}
${message}`;
    }).with({ _tag: "unparsed" }, ({ message, reason }) => {
      const detailsHeader = (0, import_chunk_DJGRU4RH.red)((0, import_chunk_DJGRU4RH.bold)("Details:"));
      return `${reason}
${detailsHeader} ${message}`;
    }).exhaustive();
    const errorMessageWithContext = `${constructedErrorMessage}
[Context: getDmmf]`;
    super((0, import_chunk_XLAUU25L.addVersionDetailsToErrorMessage)(errorMessageWithContext));
    this.name = "GetDmmfError";
  }
};
async function getDMMF(options) {
  warnOnDeprecatedFeatureFlag(options.previewFeatures);
  const debugErrorType = (0, import_chunk_M2BC5C3V.createDebugErrorType)(debug, "getDmmfWasm");
  debug(`Using getDmmf Wasm`);
  const dmmfPipeline = (0, import_function.pipe)(
    (0, import_chunk_YJ2WM7K5.tryCatch2)(
      () => {
    ...
    (0, import_chunk_YJ2WM7K5.chainW2)((datamodel) => {
      return (0, import_function.pipe)(
        (0, import_chunk_YJ2WM7K5.tryCatch)(
          () => {
            if (process.env.FORCE_PANIC_QUERY_ENGINE_GET_DMMF) {
              debug("Triggering a Rust panic...");
              import_chunk_TTXZEUOA.prismaSchemaWasm.debug_panic();
            }
            const params = JSON.stringify({
              prismaSchema: datamodel,
              noColor: Boolean(process.env.NO_COLOR)
            });
            const data = import_chunk_TTXZEUOA.prismaSchemaWasm.get_dmmf(params);
            return data;
          },
          (e) => ({
            type: "wasm-error",
        ...
        (0, import_chunk_YJ2WM7K5.map)((result) => ({ result })),
        (0, import_chunk_YJ2WM7K5.chainW)(
          ({ result }) => (
  ...
  if ((0, import_chunk_YJ2WM7K5.isRight)(dmmfEither)) {
    debug("dmmf data retrieved without errors in getDmmf Wasm");
    const { right: data } = dmmfEither;
    return Promise.resolve(data);
  }
  const error = (0, import_chunk_KTERFAQE.N)(dmmfEither.left).with({ type: "read-datamodel-path" }, (e) => {
    debugErrorType(e);
...
Datamodel path: "${e.datamodelPath}"`,
    ...
    return new GetDmmfError((0, import_chunk_M2BC5C3V.parseQueryEngineError)({ errorOutput, reason: e.reason }));
...
function warnOnDeprecatedFeatureFlag(previewFeatures) {
  const getMessage = (flag) => `${(0, import_chunk_DJGRU4RH.blue)((0, import_chunk_DJGRU4RH.bold)("info"))} The preview flag "${flag}" is not needed anymore, please remove it from your schema.prisma`;
