...
var chunk_AKMD3EDB_exports = {};
__export(chunk_AKMD3EDB_exports, {
  exists: () => exists,
  loadEnv: () => loadEnv,
  pathsEqual: () => pathsEqual,
  tryLoadEnvs: () => tryLoadEnvs
});
module.exports = __toCommonJS(chunk_AKMD3EDB_exports);
...
var import_dotenv = (0, import_chunk_FVJ3R4NJ.__toESM)(require_main());
...
function tryLoadEnvs({
  ...
  if (schemaEnvInfo?.dotenvResult.error) {
    return console.error((0, import_chunk_DJGRU4RH.red)((0, import_chunk_DJGRU4RH.bold)("Schema Env Error: ")) + schemaEnvInfo.dotenvResult.error);
  }
...
function checkForConflicts(rootEnvInfo, envPath, type) {
  ...
  if (parsedRootEnv && envPath && areNotTheSame && import_fs.default.existsSync(envPath)) {
    ...
    if (conflicts.length > 0) {
      ...
      if (type === "error") {
        const message = `There is a conflict between env var${conflicts.length > 1 ? "s" : ""} in ${(0, import_chunk_DJGRU4RH.underline)(
          relativeRootEnvPath
        )} and ${(0, import_chunk_DJGRU4RH.underline)(relativeEnvPath)}
Conflicting env vars:
${conflicts.map((conflict) => `  ${(0, import_chunk_DJGRU4RH.bold)(conflict)}`).join("\n")}

We suggest to move the contents of ${(0, import_chunk_DJGRU4RH.underline)(relativeEnvPath)} to ${(0, import_chunk_DJGRU4RH.underline)(
          relativeRootEnvPath
        )} to consolidate your env vars.
`;
        throw new Error(message);
      } else if (type === "warn") {
        const message = `Conflict for env var${conflicts.length > 1 ? "s" : ""} ${conflicts.map((c) => (0, import_chunk_DJGRU4RH.bold)(c)).join(", ")} in ${(0, import_chunk_DJGRU4RH.underline)(relativeRootEnvPath)} and ${(0, import_chunk_DJGRU4RH.underline)(relativeEnvPath)}
Env vars from ${(0, import_chunk_DJGRU4RH.underline)(relativeEnvPath)} overwrite the ones from ${(0, import_chunk_DJGRU4RH.underline)(relativeRootEnvPath)}
      `;
        console.warn(`${(0, import_chunk_DJGRU4RH.yellow)("warn(prisma)")} ${message}`);
      }
    }
  }
}
function loadEnv(envPath) {
  if (exists(envPath)) {
    ...
    return {
      ...
      message: (0, import_chunk_DJGRU4RH.dim)(`Environment variables loaded from ${import_path.default.relative(process.cwd(), envPath)}`),
      path: envPath
    };
  } else {
