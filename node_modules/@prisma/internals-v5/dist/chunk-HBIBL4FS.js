...
var chunk_HBIBL4FS_exports = {};
__export(chunk_HBIBL4FS_exports, {
  handleLibraryLoadingErrors: () => handleLibraryLoadingErrors
});
module.exports = __toCommonJS(chunk_HBIBL4FS_exports);
...
function handleLibraryLoadingErrors(args) {
  ...
  const errorTitle = `Unable to require(\`${(0, import_chunk_DJGRU4RH.dim)(args.id)}\`).`;
  const potentialReasonMessage = (0, import_chunk_KTERFAQE.N)({ message: error.message, code: error.code }).with({ code: "ENOENT" }, () => `File does not exist.`).when(
