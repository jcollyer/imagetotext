...
var chunk_LXMH457V_exports = {};
__export(chunk_LXMH457V_exports, {
...
module.exports = __toCommonJS(chunk_LXMH457V_exports);
...
var import_execa = (0, import_chunk_FVJ3R4NJ.__toESM)((0, import_chunk_J2RYL4JN.require_execa)());
...
async function getPrismaConfigFromPackageJson(cwd) {
  const pkgJson = await (0, import_chunk_KXIPL557.readPackageUp)({ cwd, normalize: false });
...
async function getSchema(schemaPathFromArgs) {
  ...
  if (!schemaPath) {
    throw new Error(
      `Could not find a ${(0, import_chunk_DJGRU4RH.bold)(
        "schema.prisma"
      )} file that is required for this command.
You can either provide it with ${(0, import_chunk_DJGRU4RH.green)(
        "--schema"
      )}, set it as \`prisma.schema\` in your package.json or put it into the default location ${(0, import_chunk_DJGRU4RH.green)(
...
function getSchemaPathFromPackageJsonSync(cwd) {
  const pkgJson = (0, import_chunk_KXIPL557.readPackageUpSync)({ cwd, normalize: false });
