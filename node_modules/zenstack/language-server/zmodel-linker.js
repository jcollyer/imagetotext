...
class ZModelLinker extends langium_1.DefaultLinker {
    ...
    linkReference(container, property, document, extraScopes, onlyFromExtraScopes = false) {
    ...
    resolveReference(node, document, extraScopes) {
        ...
        if (node.target.ref) {
            ...
            collectionPredicateContext &&
                ...
                (0, ast_1.isDataModelField)(node.target.ref) &&
    ...
    getCollectionPredicateContextDataModel(node) {
        var _a, _b;
        let curr = node;
        while (curr) {
            if (curr.$container &&
                // parent is a collection predicate
                (0, ast_utils_1.isCollectionPredicate)(curr.$container) &&
                // the collection predicate's LHS is resolved to a DataModel
                (0, ast_1.isDataModel)((_a = curr.$container.left.$resolvedType) === null || _a === void 0 ? void 0 : _a.decl) &&
                // current node is the RHS
                curr.$containerProperty === 'right') {
                // return the resolved type of LHS
                return (_b = curr.$container.left.$resolvedType) === null || _b === void 0 ? void 0 : _b.decl;
            }
            curr = curr.$container;
        }
        return undefined;
    ...
    resolveInvocation(node, document, extraScopes) {
        ...
        if (node.function.ref) {
            ...
            if (funcDecl.name === 'auth' && (0, sdk_1.isFromStdlib)(funcDecl)) {
                ...
                if (model) {
                    let authModel = (0, ast_utils_1.getAllDeclarationsFromImports)(this.langiumDocuments(), model).find((d) => {
                        return (0, ast_1.isDataModel)(d) && (0, sdk_1.hasAttribute)(d, '@@auth');
                    });
                    if (!authModel) {
                        authModel = (0, ast_utils_1.getAllDeclarationsFromImports)(this.langiumDocuments(), model).find((d) => {
                            return (0, ast_1.isDataModel)(d) && d.name === 'User';
                        });
                    }
                    if (authModel) {
    ...
    resolveMemberAccess(node, document, extraScopes) {
        ...
        if (operandResolved && !operandResolved.array && (0, ast_1.isDataModel)(operandResolved.decl)) {
            ...
            if (node.member.ref) {
                ...
                if (node.$resolvedType && (0, ast_utils_1.isAuthInvocation)(node.operand)) {
    ...
    resolveDataModel(node, document, extraScopes) {
        if (node.superTypes.length > 0) {
            ...
            while (queue.length > 0) {
                ...
                const provider = (name) => superType.fields.find((f) => f.name === name);
