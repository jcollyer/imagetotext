...
class PolicyGenerator {
        ...
        models.forEach((model) => {
            ...
            allExpressions.forEach((rule) => {
                (0, langium_1.streamAst)(rule).forEach((node) => {
                    if ((0, ast_1.isMemberAccessExpr)(node) && (0, ast_utils_1.isAuthInvocation)(node.operand)) {
                        authRules.push(node);
                    }
                });
            });
    ...
    generateSelectForRules(rules, forAuthContext = false) {
        ...
        const visit = (node) => {
            ...
            if ((0, ast_1.isMemberAccessExpr)(node)) {
                if (forAuthContext && (0, ast_utils_1.isAuthInvocation)(node.operand)) {
    ...
    generateNormalizedAuthRef(model, allows, denies, statements) {
        ...
        if (hasAuthRef) {
            ...
            if (!authModel) {
                throw new sdk_1.PluginError(_1.name, 'Auth model not found');
            }
            const userIdFields = (0, ast_utils_1.getIdFields)(authModel);
